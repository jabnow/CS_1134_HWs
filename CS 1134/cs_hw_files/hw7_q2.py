...
# question 2
    ...
    def leaves_list(self):
        """ When called on a tree, it will create and return a list, containing the values stored at
    the leaves of the tree, ordered from left to right.
    For example, if called on the tree from question 1, it should return: [5, 1, 8, 4]
        1. Your method should run in linear time.
    Hint: To meet this requirement you may want to define a generator that yields
    the desired values. You could then turn it to a list, using the list constructor.
    ...
    tree, using any of the traversals.
        ...
        def generate_leaves(root):
            if root.left is None and root.right is None:
                ...
                yield from generate_leaves(root.right)
            elif root.left is not None and root.right is None:
                yield from generate_leaves(root.left)
        ...
        if self.root is None:
            return []
        return [node.data for node in generate_leaves(self.root)]

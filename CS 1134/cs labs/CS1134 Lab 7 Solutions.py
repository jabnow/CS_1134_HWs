...
def split_parity(lst, low, high):
    if low <= high: # exit condition, couldv'e also written if low >= high: return else:
...
#and sorts them in place(duplicates allowed)

def SortLst(lst):
    count = [0] * (max(lst) + 1)  # Create a count array to store the count of each element
    
    for i in lst:
        count[i] += 1  # Count each element in lst
...
lst = [2, 0, 2, 1, 1, 0]
SortLst(lst)
print(lst)  # Output is [0, 0, 1, 1, 2, 2]
